...
async function validateCardTotals(userId, issues) {
  ...
  for (const periodStr of periods) {
    ...
    for (const row of invoiceRows) {
      ...
      const placeholderOnly = monthExpenses.filter((e) => e.name === card.name && purchases.length === 0);
      const purchaseSum = purchases.reduce((sum, e) => sum + Number(e.amount || 0), 0);
      const placeholderSum = placeholderOnly.reduce((sum, e) => sum + Number(e.amount || 0), 0);
